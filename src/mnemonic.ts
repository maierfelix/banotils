import {bytesToHex, bytesToBits} from "./utils";
 ...
 */
function isValidEntropy(entropy: Uint8Array): boolean {
  return (
    (entropy.length >= 16) &&
    (entropy.length <= 32) &&
    (entropy.length % 4 === 0)
...
export async function entropyToMnemonic(entropy: Uint8Array): Promise<string> {
  if (!isValidEntropy(entropy)) throw new TypeError(`Invalid entropy '${bytesToHex(entropy)}'`);
...
export async function mnemonicToEntropy(mnemonic: string): Promise<Uint8Array> {
  ...
  if (words.length % 3 !== 0) throw new Error(`Invalid mnemonic '${mnemonic}'`);

  // Convert word indices to 11 bit binary strings
  const bits = words.map(
    (word: string): string => {
      const index = ENGLISH_WORDLIST.indexOf(word);
      if (index === -1) throw new Error(`Invalid mnemonic '${mnemonic}'`);
  ...
  const entropy = new Uint8Array(entropyBits.match(/(.{1,8})/g)!.map((v: string) => parseInt(v, 2)));
  ...
  if (newChecksumBits !== checksumBits) throw new Error(`Invalid checksum '${checksumBits}'`);
